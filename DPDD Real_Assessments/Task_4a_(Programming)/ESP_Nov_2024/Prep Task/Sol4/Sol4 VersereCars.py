...
def brand_sales_overtime():
    ...
    df1 = df[['Date', 'Brand', 'Value']]
    df1 = df1[df1['Brand'] == brandchoice]
    df1 = df[['Date', 'Value']]
    df1.set_index('Date', inplace=True)
    df1.plot.bar()
    title = brandchoice + " Sales over time"
    plt.title(title)
    plt.show()
...
def model_sales_overtime():
    ...
    df1 = df[['Date', 'Car Model', 'Value']]
    df1 = df1[df1['Car Model'] == modelchoice]
    df1 = df[['Date', 'Value']]
    df1.set_index('Date', inplace=True)
    df1.plot.bar()
