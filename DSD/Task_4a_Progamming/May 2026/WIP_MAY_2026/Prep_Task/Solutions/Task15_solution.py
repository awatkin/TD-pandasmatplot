...
def main_menu():
    ...
    while flag:
        ...
        print("### 8. All Earnings for Each Athlete for one event - Task 7")
        print("### 9. Finish type count for one specific Athlete - Task 8")
        print("### 15. Average Winnings for each place in one Event - Task 15")
...
# new subroutine for task 6
def athlete_earnings_one_event(menu_choice):
    df = read_df()
    df = df[df['Event'] == menu_choice]
    df = df[['Athlete', 'Prize Money']]
    df = df.groupby("Athlete").sum()
...
# new subroutine for task 8
def finish_count_for_athlete(menu_choice):
    df = read_df()
    df = df[df['Athlete'] == menu_choice]
...
def averager_winning_per_place(menu_choice):
    df = read_df()
    df = df[df['Event'] == menu_choice]
    df = df[['Place', 'Prize Money']]
    df = df.groupby('Place').mean()
...
def main():
    ...
    while chkflg:
        ...
        if main_menu_choice == "1":
            ...
            menu_choice = event_picker()
            earn = athlete_earnings_one_event(menu_choice)
            grapher(earn, "Earnings for each Athlete for one event", "Value in £")
            print("The Earnings per Athelete")
            print(earn)

        elif main_menu_choice == "9":
            menu_choice = athlete_picker()
            finishes = finish_count_for_athlete(menu_choice)
            grapher(finishes, "Count of each Finish Type", "No. Of Finishes")
            print("The Number of times each Finish was achieved:")
            print(finishes)

        elif main_menu_choice == "15":
            menu_choice = event_picker()
            avgwin = averager_winning_per_place(menu_choice)
            grapher(avgwin, "Average Winnings Per Place", "Value in £")
            print(f"The Average winnings per place in the event {menu_choice}")
            print(avgwin)
